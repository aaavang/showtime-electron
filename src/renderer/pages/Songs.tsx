...
export function Songs() {
  ...
  const validateLibrary = async () => {
    ...
    const duplicateSongs = songs!.filter((song, index, self) =>
      self.some((s) => s.path === song.path && s.id !== song.id),
    );
      ...
      for (const song of duplicateSongs) {
        ...
        if (variants.length === 0) {
          toast({
            ...
            description:
              'This song is not associated with any dance variants. Deleting.',
            status: 'warning',
            duration: 2000,
            isClosable: true,
          });
  ...
  const saveNewSong = async () => {
    ...
    const existingSongs = await database.songs
      .where('path')
      .equals(newSong.path)
      .toArray();
    ...
    if (
      ...
      validation.invalidSongs.some((s) => s.id === newSong.id)
    ) {
      setValidation({
        ...validation,
        invalidSongs: validation.invalidSongs.filter(
          (s) => s.id !== newSong.id,
        ),
      });
    }
  ...
  const deleteSong = async (id: number) => {
    ...
    if (variants.length > 0 && defaultVariants.length === 0) {
      confirmAction(
        `Delete ${variants.length} associated variants?`,
        async () => {
    ...
    <Page name="Songs">
      <VStack height="100%" spacing={0}>
        <HStack w="100%" flexShrink={0} pb={2}>
          <Box flex={1}>
            <Button colorScheme="green" onClick={newSongModal.onOpen}>
              + New Song
            </Button>
          </Box>
          <Center flex={1}>
      ...
      <Modal isOpen={newSongModal.isOpen} onClose={newSongModal.onClose}>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            ...
            <Button variant="ghost" onClick={newSongModal.onClose}>
              Cancel
            </Button>
          </ModalFooter>
        </ModalContent>
