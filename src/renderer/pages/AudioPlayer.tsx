...
export function AudioPlayer(props: any) {
  ...
  return (
    <VStack w="100%">
      {audio}
      <pre>
        {formatSecondsToLabel(state.time)}/
        {formatSecondsToLabel(state.duration)}
      </pre>
      <Progress
        hasStripe
      ...
      <HStack w="100%" justifyContent="space-between">
        <Button id="play-pause-button" onClick={handlePlayPause} color="green">
          {state.playing && !state.paused ? 'Pause' : 'Play'}
        </Button>
        <Button
          id="fade-button"
          onClick={handleFade}
          color="blue"
          isDisabled={!state.playing || state.paused}
        >
          Fade out (2.5 secs)
        </Button>
        <Button
          ...
          color="red"
          variant="outline"
          isDisabled={state.playing || state.time === 0}
        >
          Restart
        </Button>
